...
export const store = new Vue({
    ...
    methods: {
        ...
        /**
         * Returns the immutability lock state of an ADR, derived from the
         * repository's manifest (if one was loaded). An ADR is "locked" once it
         * has been accepted and its content hash recorded in the manifest.
         *
         * @param {object} adr - an ADR file object (must have a `path`)
         * @returns {{locked: boolean, acceptedSha: (string|null), status: (string|null)}}
         */
        getLockState(adr) {
            const repo = this.currentRepository;
            const fallback = { locked: false, acceptedSha: null, status: null };
            if (!repo || !repo.manifest || !adr) {
                return fallback;
            }
            const record = Object.values(repo.manifest.records || {}).find((r) => r.path === adr.path);
            if (!record) {
                return fallback;
            }
            return {
                locked: record.status === "accepted" && Boolean(record.acceptedSha),
                acceptedSha: record.acceptedSha || null,
