...
import { parseManifest, manifestPath, createEmptyManifest } from "./manifest.js";
...
/**
 * Returns the commit history for a single file, newest first. Each entry is one
 * immutable point in the file's Git history and carries the commit SHA, author
 * and date. Combined with {@link loadBlobBySha} / {@link loadRawFileAtRef} this
 * lets the UI present any past *version* of an ADR addressed by its hash.
 *
 * See 'https://docs.github.com/en/rest/commits/commits#list-commits' (uses the
 * `path` and `sha` query parameters to scope to one file on one branch).
 *
 * @param {string} repoFullName - e.g. 'adr/madr'
 * @param {string} branch - branch name to scope the history to
 * @param {string} filePath - e.g. 'docs/decisions/0001-some-name.md'
 * @param {number} perPage - maximum number of commits to return
 * @returns {Promise<object[]>} array of commit objects ({ sha, commit: { author, message }, ... })
 */
export async function loadFileCommitHistory(repoFullName, branch, filePath, perPage = 50) {
    return axios
        .get(`${BASE_URL_REPO}/${repoFullName}/commits`, {
            params: { path: filePath, sha: branch, per_page: perPage }
        })
        .then((response) => response.data)
        .catch((err) => {
            console.log(err);
...
/**
 * Loads a Git blob directly by its content hash (blob SHA). This is the
 * content-addressable read path: the same content always lives under the same
 * SHA, and the SHA is verifiable with {@link verifyContentHash}.
 *
 * See 'https://docs.github.com/en/rest/git/blobs#get-a-blob'.
 *
 * @param {string} repoFullName - e.g. 'adr/madr'
 * @param {string} sha - the 40-character Git blob SHA
 * @returns {Promise<string>} the decoded UTF-8 content of the blob
 */
export async function loadBlobBySha(repoFullName, sha) {
    return axios
        .get(`${BASE_URL_REPO}/${repoFullName}/git/blobs/${sha}`)
        .then((response) => response.data)
        .then((data) => decodeUnicode(data.content))
        .catch((err) => {
            console.log(err);
        });
}
...
/**
 * Loads the raw content of a file at an arbitrary Git ref (a branch name, tag,
 * or commit SHA). Used to display a historical version of an ADR.
 *
 * @param {string} repoFullName - e.g. 'adr/madr'
 * @param {string} ref - a branch, tag or commit SHA
 * @param {string} filePath - e.g. 'docs/decisions/0001-some-name.md'
 * @returns {Promise<string>} the raw content of the file at that ref
 */
export async function loadRawFileAtRef(repoFullName, ref, filePath) {
    return loadRawFile(repoFullName, ref, filePath);
}
...
export async function loadARepositoryContent(repoFullName, branchName) {
    ...
    repoPromises.push(
        loadFileTreeOfRepository(repoFullName, branchName).then((data) => {
            ...
            // Load the immutability manifest (if present) so the UI can show
            // lock state and version hashes. A missing manifest is fine.
            adrPromises.push(
                loadRawFile(repoFullName, branchName, manifestPath(repoObject.adrPath))
                    .then((rawManifest) => {
                        repoObject.manifest = rawManifest ? parseManifest(rawManifest) : createEmptyManifest();
                    })
                    .catch(() => {
                        repoObject.manifest = createEmptyManifest();
                    })
            );
