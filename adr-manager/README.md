...
## Fork addition: ADR immutability via Git's content-addressable storage

This fork adds the ability to make accepted ADRs **immutable**, leveraging Git's
existing content-addressable object store rather than introducing new storage:

- A **"Version History"** tab shows every version of an ADR addressed by its Git
  blob/commit SHA, and verifies a version's content hash against the locked
  accepted hash.
- An append-only **manifest** (`<adrPath>/.adr-manifest.json`) indexes versions
  by content hash and records lifecycle + cross-link (parent/child, supersedes)
  metadata.
- A **CI check** (`npm run verify:immutability`) plus **branch protection** and
  **signed commits** enforce immutability on shared branches.

See [`docs/immutability/README.md`](docs/immutability/README.md) for the full
...
- [adr.zone](https://www.adr.zone/adr-generator?format=madr) supports form-based ADR generation.
